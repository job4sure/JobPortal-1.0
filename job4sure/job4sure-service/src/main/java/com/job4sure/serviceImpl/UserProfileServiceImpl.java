...
public class UserProfileServiceImpl implements UserProfileService {
			...
			if (upload != null && upload.length > 0) {
				for (CommonsMultipartFile multipartFile : upload) {
					...
					try {
						...
						UserProfile currentUserProfile = profileCompleteRepository
								.save(userProfile);
						File newFile = new File(IConstant.FILE_PATH);
						File filePath = new File(newFile + File.separator
								+ currentUserProfile.getRegistrationId() + "_"
								+ attchmentName);
						if (!newFile.exists()) {
							newFile.mkdir();
						...
						if (profilePic.equals("jpg")
							...
							oldAttchment = attachmentRepository
									.getProfilePicAttachment(currentUserProfile
											.getRegistrationId(), attchmentType);
							...
							attchment.setPath(newFile + File.separator
									+ currentUserProfile.getRegistrationId()
									+ "_" + attchmentName);
							attchment.setRegistrationId(currentUserProfile
									.getRegistrationId());
	...
	public Attachment getProfilePic(Integer registrationId) {
		...
		if (registrationId != null) {
			String attchmentType = "profilePic";
			Attachment profilepicattachment = attachmentRepository
					.getProfilePicAttachment(registrationId, attchmentType);
	...
	public boolean getUserResume(Integer registrationId,
			HttpServletResponse response) throws IOException {
		String attchmentType = "resume";
		boolean status = false;
		Attachment attachment = attachmentRepository.getProfilePicAttachment(
				registrationId, attchmentType);
		if (attachment != null) {
			DownloadResume.downloadResume(attachment.getAttachmentName(),
