...
function searchByName(){
	 ...
	 $("#toMailId"+${user.registrationId}).hide();
   </c:forEach>  
 ...
 textValue=textValue.toLowerCase();
  ...
  i=0;
 if(status==0){
	 $("#toMailId"+${user.registrationId}).show();
 }
    </c:forEach>  
}
			...
			<form:form modelAttribute="sendMail" action="sendMailToGroup" method="POST" id="formID">
				<div id="content" class="box">
					...
					<div id="content" class="box">
						...
						<c:forEach items="${users}" var="user">
								<div id="toMailId${user.registrationId}">
									<form:checkbox path="toMailId" value="${user.email}" />
									<label><b>Name:</b> ${user.fullName} &nbsp;&nbsp; <b>Email Id:</b> ${user.email}</label> <br>
								</div>
							</c:forEach>
						</div>
					</div>
